...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const baa = localStorage.getItem('hr_baa_signed')
  ...
  const handleSignBAA = () => {
    localStorage.setItem('hr_baa_signed', 'true')
    setBaaSigned(true)
    setShowBAA(false)
  ...
  const handleSubmitAudit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/run-audit', {
        ...
        body: JSON.stringify({
          endpointUrl: auditForm.url,
          apiKey: auditForm.apiKey,
        }),
      })
        ...
        <div className="max-w-[1280px] mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="relative">
            <button onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center gap-2 text-[#E9EEF5] hover:text-white transition-colors">
              <div className="w-8 h-8 rounded-full bg-[#14B8A6]/20 border border-[#14B8A6]/40 flex items-center justify-center text-[#14B8A6] font-black text-sm" style={F}>
            ...
            {dropdownOpen && (
              <div className="absolute right-0 top-12 bg-[#111827] border border-[#374151] rounded-lg py-2 w-40 shadow-xl z-50">
                <button
                  onClick={() => { localStorage.removeItem('hr_user'); router.push('/') }}
            ...
            <form onSubmit={handleSubmitAudit} className="space-y-4">
              ...
              <div>
                ...
                <input required type="password"
                  placeholder="Your temporary API key"
                  className="w-full bg-[#080C14] border border-[#1F2937] rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-[#14B8A6] transition-colors"
                  style={F}
              ...
              <div>
                ...
                <textarea
                  placeholder="e.g. Epic MyChart patient portal chatbot"
              ...
              {auditError && (
                <p className="text-red-400 text-sm font-semibold" style={F}>{auditError}</p>
              )}
